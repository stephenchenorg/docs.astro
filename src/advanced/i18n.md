...
在 GraphQL 請求中設定語系變數，只需在 `src/api/index.ts` 的 `defaultVariables` 加入語系變數即可，但需注意的是，前端習慣使用 `-` 連接語系的格式，後端使用 `_` 來連接，因此需要轉換過才能使用：
...
然後在 GraphQL 請求中使用語系變數：

```ts
const data = await graphQLAPI<Data>(gql`
  query ($lang: String!) {
    page(key: "home", lang: $lang) {
      ...PageFields
    }
...
在 RESTful API 請求中設定 `Content-Language` header，回傳的表單驗證錯誤訊息會依據語系來顯示：
...
await fetch('/api/contacts', {
  ...
  headers: {
    ...
    'Content-Language': (data.get('lang') as string | null) || getLocale().replace('-', '_'),
